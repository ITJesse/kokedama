...
const DoujinMangaLexicon: SiteData = {
  ...
  index: 3,
  urlMatcher:
    /(?:http:\/\/)?doujinshi\.mugimugi\.org\/index\.php?P=BOOK&ID=\d+/i,
...
const Pixiv: SiteData = {
  ...
  index: 5,
  urlMatcher:
    /(?:https?:\/\/)?(?:www\.)?pixiv\.net\/member_illust\.php\?mode=.+&illust_id=\d+/i,
...
const Fakku: SiteData = {
  ...
  backupUrl: (data) =>
    `https://www.fakku.net/hentai/${data.data.source
      ?.toLowerCase()
      .replace(' ', '-')}`,
}
...
const AniDB: SiteData = {
  ...
  index: 21,
  urlMatcher:
    /(?:https?:\/\/)?anidb\.net\/perl-bin\/animedb\.pl\?show=.+&aid=\d+/i,
  backupUrl: (data) =>
...
const Gelbooru: SiteData = {
  ...
  index: 25,
  urlMatcher:
    /(?:https?:\/\/)gelbooru\.com\/index\.php\?page=post&s=view&id=\d+/i,
  backupUrl: (data) =>
...
const PortalGraphics: SiteData = {
  ...
  index: 33,
  urlMatcher:
    /(?:http:\/\/)?web\.archive\.org\/web\/http:\/\/www\.portalgraphics\.net\/pg\/illust\/\?image_id=\d+/i,
  backupUrl: (data) =>
...
const EHentai: SiteData = {
  name: 'E-Hentai',
  index: 38,
  urlMatcher: /(?:https:\/\/)?e-hentai\.org\//gi,
  backupUrl: (data) =>
    `https://e-hentai.org/?f_search=${encodeURIComponent(
      data.data.source ?? '',
    )}`,
  authorData: ({ creator }) => ({
    authorName: creator?.[0] ?? '',
    authorUrl: creator?.[0]
      ? `https://e-hentai.org/tag/artist:${encodeURIComponent(creator[0])}`
      : '',
  }),
}
const FurAffinity: SiteData = {
  name: 'FurAffinity',
  index: 40,
  urlMatcher: /(?:https:\/\/)?www\.furaffinity\.net\/view\/\d+/gi,
  backupUrl: (data) => `https://www.furaffinity.net/view/${data.data.fa_id}`,
  authorData: ({ author_name, author_url }) => ({
